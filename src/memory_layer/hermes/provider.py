...
and activates it with ``memory.provider: memorylayer``. Once active it replaces
...
arrive from the ``memorylayer_remember`` tool, from mirrored built-in memory
...
from memory_layer.hermes._base import (
    INDICATOR_GLYPH,
    MemoryProvider,
    RecallStatus,
    is_trivial_prompt,
)
from memory_layer.hermes.bridge import run_sync, spawn
from memory_layer.hermes.tools import TOOL_SCHEMAS, dispatch

if TYPE_CHECKING:
    from memory_layer.core.engine import EngineStats, MemoryEngine
    from memory_layer.core.models import Memory, SearchResult

logger = get_logger(__name__)

PROVIDER_NAME = "memorylayer"
PROVIDER_LABEL = "Memory Layer"
...
class MemoryLayerProvider(MemoryProvider):
    ...
    def __init__(self) -> None:
        ...
        self._db_path: Path = Path(os.environ.get("MEMORY_LAYER_DB", DEFAULT_DB_PATH)).expanduser()
        self._recall_limit: int = int(
            os.environ.get("MEMORY_LAYER_RECALL_LIMIT", DEFAULT_RECALL_LIMIT)
        )
        self._min_score: float = float(os.environ.get("MEMORY_LAYER_MIN_SCORE", DEFAULT_MIN_SCORE))
        self._mirror_builtin: bool = _env_flag("MEMORY_LAYER_MIRROR_WRITES", True)
        self._extract_on_end: bool = _env_flag("MEMORY_LAYER_EXTRACT_ON_END", False)
        ...
        from memory_layer.hermes.trace import TraceWriter  # noqa: PLC0415
    ...
    def unavailable_reason(self) -> str:
        ...
        return f"Cannot write to {self._db_path.parent}. Set MEMORY_LAYER_DB to a writable path."
    ...
    def initialize(self, session_id: str, **kwargs: Any) -> None:
        ...
        """
        from memory_layer.core.engine import EngineConfig, MemoryEngine  # noqa: PLC0415
        ...
        self._project = os.environ.get("MEMORY_LAYER_PROJECT") or (
    ...
    def system_prompt_block(self) -> str:
        ...
        return (
            "You have persistent memory across sessions via Memory Layer. "
            "Relevant memories are retrieved automatically each turn. "
            "Use memorylayer_remember to save a durable fact worth recalling "
            "later, and memorylayer_outcome to report whether recalled memories "
    ...
    async def _extract(self, messages: list[dict[str, Any]]) -> None:
        ...
        """
        from memory_layer.extraction.extractor import MemoryExtractor  # noqa: PLC0415
    ...
    def get_config_schema(self) -> list[dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "env_var": "MEMORY_LAYER_RECALL_LIMIT",
                "type": "integer",
                "minimum": 1,
                "maximum": 50,
            },
            {
                ...
                "default": True,
                "env_var": "MEMORY_LAYER_MIRROR_WRITES",
                "type": "boolean",
            },
            {
                "key": "extract_on_end",
                ...
                "default": False,
                "env_var": "MEMORY_LAYER_EXTRACT_ON_END",
                "type": "boolean",
            },
        ]
